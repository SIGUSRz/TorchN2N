...
class AttnDecoderRNN(nn.Module):
    def __init__(self, hidden_size, output_size, output_encoding_size,
        ...
        self.assembler_w = torch.FloatTensor(assembler_w).cuda() if use_cuda else torch.FloatTensor(
            assembler_w)
        self.assembler_b = torch.FloatTensor(assembler_b).cuda() if use_cuda else torch.FloatTensor(
            assembler_b)
        self.assembler_p = torch.FloatTensor(assembler_p).cuda() if use_cuda else torch.FloatTensor(
            assembler_p)
        self.batch_size = 0
    ...
    def _get_valid_tokens(self, decoding_state, assembler_W, assembler_b):
        ...
        expanded_state = decoding_state.view(batch_size, 3, 1, 1).expand(batch_size, 3,
                                                                         self.output_size, 4)

        expanded_w = assembler_W.view(1, 3, self.output_size, 4).expand(batch_size, 3,
                                                                        self.output_size, 4)
    ...
    def _step_by_step_attention_decoder(self, time, embedded, previous_hidden_state,
        ...
        encoder_outputs_expand = encoder_outputs.view(1, seq_len, batch_size, hidden_size).expand(
            out_len, seq_len,
            batch_size,
            hidden_size)
        ...
        output_expand = output.view(out_len, 1, batch_size, hidden_size).expand(out_len, seq_len,
                                                                                batch_size,
                                                                                hidden_size)
        output_transfrom = self.decoderLinear(output_expand)

        ##raw_attention size (out_len,seq_len,batch_size,1)
        raw_attention = self.attnLinear(F.tanh(encoder_transform + output_transfrom)).view(out_len,
                                                                                           seq_len,
                                                                                           batch_size)  ## Eq2
        ...
        return predicted_token.permute(1,
                                       0), hidden, attention, updated_decoding_state, token_neg_entropy, selected_token_log_prob
    ...
    def forward(self, encoder_hidden, encoder_outputs, encoder_lens, target_variable, sample_token):
        ...
        ## set initiate step:
        time = 0
        start_token = Variable(torch.LongTensor(np.zeros((1, self.batch_size))),
                               requires_grad=False)
        start_token = start_token.cuda() if use_cuda else start_token
        next_input = self.go_embeding(start_token)
        next_decoding_state = torch.FloatTensor([[0, 0, self.max_decoder_len]]).expand(
        ...
        while time < self.max_decoder_len:
            predicted_token, previous_hidden, context, next_decoding_state, neg_entropy, log_seq_prob = \
                self._step_by_step_attention_decoder(time=time,
                                                     ...
                                                     decoding_state=next_decoding_state,
...
class attention_seq2seq(nn.Module):
    ...
    def forward(self, input_seqs, input_seq_lens, target_variable, sample_token):
        ...
        encoder_outputs, encoder_hidden, txt_embedded = self.encoder(input_seqs, input_seq_lens,
                                                                     encoder_hidden)
